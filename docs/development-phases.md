...
| JP 適時開示の**本文** | TDnet 本文は採用不可のまま（D-126）。Discovery（誰が・いつ・何の種類を開示したか）は Yanoshin API で取得できる（D-125、実 API 検証済み）。本文の Verification は issuer 公式 IR / EDINET から |
...
| news source 9件 | EDINET / SEC EDGAR / 財務省 / 経産省 / 米財務省 / BLS・Census・BEA / company IR。adapter は汎用 feed 実装で足りるが、実フィードを叩いていない（EDINET と統計3庁は無料 API キーが要る） |
...
| 4 ✅ | News / Disclosure Collection（2026-09-17 完了） | `news` スキーマ（source registry / quad-state の source policy / documents の4時刻 / raw_documents manifest / fetch cursors / coverage / purge）、stdlib のみの RSS・Atom parser、licence 先行のコレクタ、`news.documents_as_of()` | 4つの時刻が DB 制約で強制され、backfill が過去の知識にならないことをテストで示す。**14 source を規約つきで登録し、うち5件（Yanoshin TDnet / 日銀 / Federal Reserve / White House / 内閣府 ESRI）を実 API・実フィードで検証済み。残りは未検証** | RF-17（時刻記録の部分） | **完了。** `available_to_model_at` は取得時刻から導出（D-108）。全文保存は明示許諾のみ（D-109）。news は自前 manifest（D-110） |
...
| 8 | ENTRY 判断 / Watch / Prediction / Episode | **本体実装済み（`IMPLEMENTED_NOT_LIVE_VERIFIED`）**: `prod` スキーマ（setups / watches / watch_transitions / episodes / entry_attempts / predictions / state_transitions / risk_line_updates / episode_outcomes）、`surge.entry`（watch state machine / entry decision / episode book / horizon）、Stage 3 → Setup の受け渡し、`ui.open_episodes` と `ui.entry_attempt_ledger` と対応する2画面。**未了**: 場中 Runner とリアルタイム Provider（D-06b）、`entry_price_method`（D-01a）、取引カレンダー、Outcome のパス解決（Phase 9） | TRIGGER_HIT → ENTERED の直行を DB trigger と Python の両方が拒否する / 3,000円を decision と entry の2回判定する / mock provider・非 INCLUDED・価格欠落から Prediction が作れない / Episode が thesis ごとに1本 / horizon が REAFFIRMED でリセットされない、をテストで示す | RF-03, RF-06, RF-07, RF-09（ENTRY 部分）, RF-19, RF-20, RF-21（記録部分） | **D-06b（live 検証の blocker）**, D-01a, D-01b, D-17a, D-17b, D-20, D-21, D-31 |
| 9 | Outcome / パス解決 / 教師データ前段 | **実装済み（`IMPLEMENTED_NOT_LIVE_VERIFIED`）**: `surge.outcome`（5段のパス解決ラダー、comparable path、二層 Outcome、S0〜S20、**S20 未到達は pending で確定させない**）、`prod.close_episode_with_outcome()` で close と outcome を1呼び出しに、`ui.episode_results` / `ui.outcome_counts` と Results 画面。**未了**: 実価格データ全般、分足・約定 Provider | AMBIGUOUS_PATH と UNRESOLVED_MISSING_DATA を区別する / S0 が entry 時刻以降だけを読む / 分割が failure にならない / 記録の無い分割形 gap を確定させない / FX 入力が存在しない、をテストで示す | RF-10, RF-18, RF-22, RF-24 | D-06b, D-08a, D-24 |
| 10 | Teacher Dataset | **実装済み（`IMPLEMENTED_NOT_LIVE_VERIFIED`）**: `labels` スキーマ（objective_labels / interpretive_labels / pipeline_miss_records / admission_policies / datasets）、`surge.labels`（測定・3分類・admission）、`ui.teacher_label_counts`。**未了**: 実 Episode が無いため実ラベルは0件 | 単一 binary を target にできない / 3クラス未満の policy を拒否する / backfill 文書を as-of 判定に見せると拒否する / B・C・LATE を学習対象にしない、をテストで示す | RF-02, RF-13, RF-17, RF-23 | **D-13a / D-13b（個々のラベル基準と採用ポリシー）**, D-33 |
| 11 | ML / Weight Learning | **器のみ実装（`IMPLEMENTED_NOT_LIVE_VERIFIED`）**: `surge.modellab`（walk-forward fold・purge gap・champion/challenger・promotion gate）。**Production model は学習も昇格もしていない**。実 Episode が0件なので実教師データも0件で、gate は現在すべての promotion を理由付きで拒否する（`test_today_the_gate_refuses_everything` が assert）。**未了**: 4層の学習、条件付き Weight、Feature interaction、calibration | シャッフル分割の引数が存在しない / purge gap が horizon 未満なら例外 / 学習ラベルの決着が test window 内なら検出する / 同一 fold 以外の champion 比較を拒否する / 引き分けを勝ちにしない、をテストで示す | RF-15, RF-16 | **D-13b（採用ポリシー）**, 十分な実教師データ |
| 12 | Model Lab / Continuous Improvement | **実装済み（全 surface 0 件）**: `research`（episode_attribution / model_versions / walk_forward_runs / fold_results / calibration_bins / promotion_attempts）、`ui` 10 view（teacher 集計・見逃し3分類・route/driver/concept 別成績・LLM 判定監査・model version 比較・walk-forward・calibration・promotion 監査ログ）、Improvement と Model lab の2画面。**未了**: 実データ（0 件が正しい状態）、Feature 別成績の粒度 | 20 surface すべてが 0 行であることを assert / 教師 table が 0 件であることを assert / purge gap 30 日未満を拒否 / 理由なしの refusal を拒否 / 未測定 fold を 0.00 と表示しない / Challenger の昇格が監査ログ付きで行える | RF-15 | 実運用開始（D-102 / D-103 / D-06b）、D-13b |
