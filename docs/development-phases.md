...
| 9 | Outcome / パス解決 / 教師データ前段 | **実装済み（`IMPLEMENTED_NOT_LIVE_VERIFIED`）**: `surge.outcome`（5段のパス解決ラダー、comparable path、二層 Outcome、S0〜S20）、`prod.close_episode_with_outcome()` で close と outcome を1呼び出しに、`ui.episode_results` / `ui.outcome_counts` と Results 画面。**未了**: 実価格データ全般、分足・約定 Provider | AMBIGUOUS_PATH と UNRESOLVED_MISSING_DATA を区別する / S0 が entry 時刻以降だけを読む / 分割が failure にならない / 記録の無い分割形 gap を確定させない / FX 入力が存在しない、をテストで示す | RF-10, RF-18, RF-22, RF-24 | D-06b, D-08a, D-24 |
