...
| JP 適時開示（TDnet） | **採用不可が確定**（D-122）。robots.txt の全ホスト禁止 + 複製禁止条項。EDINET は代替にならない |
...
| 4 ✅ | News / Disclosure Collection（2026-09-17 完了） | `news` スキーマ（source registry / quad-state の source policy / documents の4時刻 / raw_documents manifest / fetch cursors / coverage / purge）、stdlib のみの RSS・Atom parser、licence 先行のコレクタ、`news.documents_as_of()` | 4つの時刻が DB 制約で強制され、backfill が過去の知識にならないことをテストで示す。**13 source を規約つきで登録し、うち4件（日銀 / Federal Reserve / White House / 内閣府 ESRI）を実フィードで検証済み。残りは未検証** | RF-17（時刻記録の部分） | **完了。** `available_to_model_at` は取得時刻から導出（D-108）。全文保存は明示許諾のみ（D-109）。news は自前 manifest（D-110） |
