...
**状態（2026-09-19 21:10 JST）**: cohort 作成済み（frozen fingerprint `73cceb0d…`。Canonical・addenda 5 本・question schema の hash が MANIFEST とコードに一致し、`jev-1.13.0`・2026-09-24・25 営業日・$2.50 で固定されていることを確認。D-278 の変更の前後でも同一）。Task Scheduler の `Surge` フォルダに 2 つの task を登録済み: prediction `surge-jev-phase-b-jev-phase-b-jp-20260924-v1`（平日 16:10）と outcome `surge-jev-phase-b-outcome-jev-phase-b-jp-20260924-v1`（平日 18:00）。どちらも repository の外に置いた専用の frozen worktree `surge-phase-b-jp-20260924-v1` を commit `3289fdf` に固定して動かし、WakeToRun・StartWhenAvailable・ログオン中のみ・IgnoreNew。次回の起動は 2026-09-21（月）で、JPX の休業日なので両方とも何もせず終了する。**最初に送信し得るのは 2026-09-24（木）16:10 JST**、最初の outcome は 9/24 の T+20 = 10/23 18:00。登録した task を手動で起動し、土曜は両方 `closed_day`（exit 0、何も取得・送信・確定しない）、別の process が cohort の lock を持つ間は両方 `locked`（exit 0）になることを確かめた。この PC の電源設定は wake timer を AC では許可・battery では不許可（変更していない）: battery ではスリープから起こさず、次に起きたときに StartWhenAvailable で追いつく（prediction は送信窓の中だけ）。
