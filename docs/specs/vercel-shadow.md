...
| 使用量（最初の probe 後） | Active CPU 1h 46m / 4h、Provisioned Memory 28.3 / 360 GB-Hrs（+4.9）、Queue Sends 249（+159）、Blob は変化なし、**Functions Storage 11.36 GB / 10 GB**（この日の deploy 分が遅れて反映され、超過が拡大） |
| deploy（ad02432、2026-09-20 21:09 JST） | 古い deployment を 39 件削除したあと、Functions Storage の 9 GB という事前条件はユーザーが解除（公式の必須条件ではなく、9 GB に下げるには他 project の rollback まで消す必要があるため）。production = `dpl_3sE4SU6Ny2pGRACj…`（`meta.commit = ad02432…`、READY、alias 2 本）。health: 凍結指紋一致・`input_building_code_sha256 = 8158344c…`（PC と同一）・o200k 可 |
| deploy 後の確認 A〜D（S0 = 2026-09-18、Blob への書き込みなし） | **A**: step 内で curl_cffi と tiktoken が動く（chart 21 本・crumb 取得・o200k 5 トークン）。**B**: 7203 を day と同じ経路で 0.197 秒・269 本（2025-08-14〜2026-09-18、終値 3025.0・出来高 46,568,200 は PC のローカル読み取りと一致）。**C**: 10 銘柄すべて成功（4 桁・新形式 130A/135A/137A・薄商い 137A=2,600 株・分割あり 9984 1:4 と 4318 1:3。値はすべて PC と一致）。**D**: 本番と同じ 150 銘柄 1 step が **56.3 秒**（step の締切 180 秒・Function 上限 300 秒に対して十分）、失敗 0・再試行 0、CPU 2.124 秒、最大メモリ 168.7 MB、1 銘柄あたり p50 0.066 秒・p95 0.11 秒、run の events 9（step 2 本）、universe は 3,700 で JPX workbook は PC と同じ `fff94dd1…`。Workflow の data written は Hobby の画面に出ない（表示なし / 不明） |
| 使用量（deploy 後） | Functions Storage 11.27 GB / 10 GB（超過のまま。ユーザー判断で A〜D は止めない）、Deployment Storage 1.68 GB / 10 GB、Fluid Active CPU 1h 52m / 4h、Blob Storage 5.56 MB / 1 GB |
| **クラウド rehearsal（S0 = 2026-09-18、2026-09-20 22:51–23:19 JST）** | `surge/rehearsal/rehearsal-jp-vercel-v1/2026-09-18`。**27 分 39 秒・96 events・Workflow storage 5 MB**。universe 3,700（JPX workbook `fff94dd1…`）、**3,700 件すべて取得・失敗 0・coverage 100%**、3,000 円以下 2,867、screener 通過 2,066、Primary 60 / Control 20 / anonymized 8 / drift 4、**request 92 本を構築（1 本も送らない）**、o200k 18,900–19,661（平均 19,203）、送ったとしての試算 $0.100。TDnet は **COMPLETE 80 / INCOMPLETE 0**（Yanoshin を Vercel から読む経路の初成功）。読み取り 25 step で 1,401.7 秒（1 step 約 56 秒）、CPU 50.5 秒、最大メモリ 102.2 MB。再読み取り 80 件 29.7 秒。build（TDnet と token 数）3 分 3 秒。Blob は advanced 15・simple 3・1.66 MB。artifacts 11 + cohort 2 |
| **PC の 9/18 リハーサルとの照合** | PC が 2026-09-19 に同じ S0 で記録した集計と、両者が持つ 11 項目すべてが一致（workbook SHA-256・universe 3,700・読み取り 3,700・失敗 0・coverage・3,000 円以下 2,867・通過 2,066・非通過 801・Route A–H の内訳・Route D の subgroup）。PC のリハーサルは抽選をしていないので、Primary / Control の比較は 9/24 が最初 |
| **画面（2026-09-20 23:20 以降）** | production は `/` に運用状態を表示（Phase B は waiting_for_start・開始日 2026-09-24・次回 9/21 16:10・model jev-1.13.0・credit $5.00・budget $2.50 の 0 消化・deployment と code hash・A〜D・cron 未観測・rehearsal の件数）。`/system`・`/runs`・`/compare`・`/demo` も実データ。合成 fixture は production の既定表示ではなくなった |
...
- 9/20 の cron の確認と 9/24 の shadow day・比較は、リポジトリ外の運用スクリプトを Claude の一時 task（各 1 回限り）が実行する。**deploy 後の確認は A→B→C→D の順**（`pre_deploy_checks.py`）: A = step 内の C 拡張、B = 1 銘柄を day と同じ経路で読む、C = 種類の異なる 10 銘柄（4 桁・新形式の英数字・出来高の大小・分割の有無）、D = 本番と同じ 150 銘柄の読み取り step 1 回。**3,700 銘柄の probe はやり直さない**（universe 全体を読むのは 9/24 の shadow day だけ）。9/24 は、production が期待する commit（ad02432）を動かし、A〜D が通っていて、その場で 1 銘柄が読めることを確かめてからでないと起動しない。**Functions Storage は条件ではなく記録**（2026-09-20 にユーザーが 9 GB の事前条件を解除。`functions_storage.record()` が deployment ごとのバンドルと最後に読んだ画面の値を day と一緒に残す）。
