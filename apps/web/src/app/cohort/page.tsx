...
export default async function Overview({ searchParams }: { searchParams: Promise<{ cohort?: string }> }) {
  ...
  return (
    <>
          ...
          <tbody>
            {[...days].reverse().map((day: Day) => {
              ...
              return (
                <tr key={day.s0}>
                  <td>
                    <DayLink s0={day.s0} which={shadow.which} />
                  </td>
                  <td>
                  ...
                  <td>
                    {day.status !== "sent" ? (
                      ...
                      <Link href={`/cohort/outcomes?s0=${day.s0}&cohort=${shadow.which}`}>{rows.filter((r) => r.resolution === "RESOLVED").length} resolved</Link>
                    ) : (
      ...
      <p className="lede" style={{ marginTop: 26 }}>
        ...
        hashes behind it are on the <Link href={cohortHref("/cohort/system", shadow.which)}>protocol</Link> screen.
      </p>
    </>
  );
}
